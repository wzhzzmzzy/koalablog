...
describe('editor File tree', () => {
  ...
  it('preserves cached descendant Files when a Prefix refresh only returns direct Files', () => {
    ...
    replaceItemsByPrefix('/', [{ ...root, content: 'fresh root' }])
    ...
    expect(editorStore.items.find(item => item.id === child.id)).toEqual(child)
